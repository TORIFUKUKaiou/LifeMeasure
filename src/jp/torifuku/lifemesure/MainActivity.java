...
public class MainActivity extends Activity {
	...
	private void setContentView(boolean init) {
		...
		/** 設定値を取得 */
		if (init) {
			mPref = super.getSharedPreferences(super.getPackageName(),
		...
		mDatePicker.init(mYear, mMonth, mDayOfMonth,
		...
		if (locale.equals(Locale.JAPAN) || locale.equals(Locale.JAPANESE)) {
			mPatternSpinner = (Spinner) super
					.findViewById(R.id.japan_phrase_spinner);
		...
		/** 広告 */
		if (mShowResultStarted) {
			com.google.ads.AdView adView = (com.google.ads.AdView) super
	...
	protected void onPause() {
		super.onPause();
		mPref.edit().putInt(KEY_YEAR, mYear).putInt(KEY_MONTH, mMonth)
				.putInt(KEY_DAY_OF_MONTH, mDayOfMonth).putInt(KEY_HOUR, mHour)
				.putInt(KEY_MINUTE, mMinute)
				.putBoolean(KEY_SAVED, mShowResultStarted)
				.putInt(KEY_PATTERN, mPattern).commit();

		/** Widget更新 */
		AppWidgetManager awm = AppWidgetManager.getInstance(this);
		RemoteViews views = mUtility.createRemoteViews(this);
		awm.updateAppWidget(new ComponentName(this,
				LifeMeasureAppWidgetProvider.class), views);
	...
	protected void onRestoreInstanceState(Bundle savedInstanceState) {
		...
		if (savedInstanceState != null) {
			mShowResultStarted = savedInstanceState
					.getBoolean("show result stated?");
		}
	}
	...
	private void showEto() {
		String[] etoArray = { "申", "酉", "戌", "亥", "子", "丑", "寅", "卯", "辰", "巳",
				"午", "未" };
		int rem = mYear % 12;
	...
	private void dataChanged() {
		GregorianCalendar baseDate = new GregorianCalendar(mYear, mMonth,
				mDayOfMonth, mHour, mMinute);
	...
	private String createMessage() {
		...
		return mUtility.createMsg(baseDate, this.getApplicationContext(),
				mPattern);
		 ...
		 * sb.append("("); sb.append(aliveDay); sb.append("日と");
		 * sb.append(aliveHour); sb.append("時間"); sb.append(aliveMinute);
		 * sb.append("分"); sb.append(aliveSecond); sb.append("秒");
		 * sb.append(")"); sb.append("を送って来ました。");
		 * 
		 * return sb.toString();
		 */
	}
